<?php
	...
	$season = Season::find($matches[1]);
	
	$filename = 'cache/season-' . $season->id . '.png';
	
	$mtime = date("Y-m-d H:i:s.", @filemtime($GLOBALS['ROOTPATH'] . $filename));
	
	// DEBUG 
	unlink($GLOBALS['ROOTPATH'] . $filename);
	
	if (!file_exists($GLOBALS['ROOTPATH'] . $filename) || !$season->modification_date || $mtime < $season->modification_date)
	{
		...
		@mkdir($GLOBALS['ROOTPATH'] . 'cache');

		imagepng($image, $GLOBALS['ROOTPATH'] . $filename);
	}
	
	imagepng(imagecreatefrompng($GLOBALS['ROOTPATH'] . $filename));
